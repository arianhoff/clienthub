...
export default function InviteClientPage() {
  ...
  useEffect(() => {
    async function loadClients() {
      ...
      if (org) {
        ...
        if (data && data.length > 0) {
          const clientId = preselectedClientId && data.find(c => c.id === preselectedClientId) 
            ? preselectedClientId 
            : data[0].id
          setSelectedClientId(clientId)
          const selectedClient = data.find(c => c.id === clientId)
    ...
    loadClients()
  }, [supabase, slug, preselectedClientId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: inviteError } = await supabase.auth.signInWithOtp({
        email: email,
        options: {
          emailRedirectTo: `${window.location.origin}/org/${slug}/portal`,
          data: {
            full_name: fullName,
            role: 'client',
      ...
      setSuccess(true)
      setInviteLink(`${window.location.origin}/org/${slug}/portal`)
